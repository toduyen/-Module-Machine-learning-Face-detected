...
class BlinkEye extends Component {
  ...
  reunderKetQuaExpression = () => {
    ...
    if(!!orri)
    {
      let Sorted = Object.entries(orri.expressions).sort((prev, next) => prev[1] - next[1]);
      return(Sorted.pop()[0])
    }
    else
    {
  ...
  displayResult = (x1) => {
    if(this.state.audioquytoc === 1)
    {
      ...
      return(
        <div>
        <label  style={{ color: 'green' }}><span role="img" aria-label="donut">🧐</span>{ketquatextview}</label><br />
        </div>
      )
    }
  }
  ...
  renderKqChongFake = (anti) => {
    ...
    render() {  
      const { hours, minutes, seconds } = this.state;
        return (
          <div className="App">
          <h1 className="display-4">EYE BLINK...</h1>
          ...
          <h1 className="mt-5"> Timer {hours}: {minutes} : {seconds} </h1>
