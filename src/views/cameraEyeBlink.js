  ...
  ketquavekhuonmatcuata = async () => {
    if (!!this.webcam.current) {
      await getDataBlink(
        this.webcam.current.getScreenshot(),
        inputSize
      ).then(fullDesc2 => this.setState({ fullDesc2 }));
    }
  };
  ketquavekhuonmatcuata2 = async () => {
    if (!!this.webcam.current) {
      await getDataBlink2(
        this.webcam.current.getScreenshot(),
        inputSize
      ).then(fullDesc3 => this.setState({ fullDesc3 }));
    }
  };
  render() {
              ...
              <div style={{ position: 'absolute' }}>
                ...
                />
                 <div className="Boxsssss">
                  <div className="Boxsssss2">
                    L
                    </div>
                    <div className="Boxsssss3">
                    R
                   </div>
                   <p style={{ color: 'green' , fontSize: 12, float: 'left' , marginLeft: 8}}>L: {fullDesc2}</p>
                   <p style={{ color: 'green' , fontSize: 12, float: 'left' ,  marginLeft: 8}}>R: {fullDesc3}</p>
                 </div>
              </div>
            ) : null}
            {!!fullDesc ? (
